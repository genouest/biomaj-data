# !/bin/bash
# Applique un makeblastdb sur plusieurs repertoire

# Arguments:
#            1 -   chemin de l'ensemble des repertoires/fichiers a appliquer le makeblastdb
...
$PATH_PROCESS_BIOMAJ/makeblastdb.sh $fileSeq $resultdir " -dbtype nucl -parse_seqids " "$4_$bdd";
if ( test $? -ne 0 ) then
    echo "Error with makeblastdb.sh." 1>&2 ;
    exit 1
fi
done
