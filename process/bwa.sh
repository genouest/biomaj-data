...
if (test $# -ne 2) then
    ...
    echo "2: output format (default=bwa) [OPTIONAL]" 1>&2
...
if ( test $hastrailing -ne 0 ) then
    ...
    outputPath="$2/bwa-index" # There is a trailing slash, give some default prefix for index files names
fi
...
echo "Launching bwa index [bwa index -p $outputPathAbs $listFile]";

# Execute bwa index
bwa index -p "$outputPathAbs" "$listFile";

buildResult=$?
if ( test $buildResult -ne 0 ) then
    echo "BWA index failed with status $buildResult" 1>&2 ;
    exit 1
fi
